...
<?php
	...
	if(!$e_user){
		$session->msg("d","ID de usuario desconocido.");
		redirect('users.php');
	}
?>
<?php
	if(isset($_POST['update'])){
		...
		if(empty($errors)){
			...
			if($result&&$db->affected_rows()===1){
				...
				$session->msg('d','No se actualizaron los datos.');
				redirect('edit_user.php?id='.(int)$e_user['id'],false);
			}
		}else{
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form action="edit_user.php?id=<?php echo (int)$e_user['id']; ?>" method="post" class="clearfix">
					...
					<div class="form-group clearfix">
						<button type="submit" name="update-pass" class="btn btn-danger pull-right" style="margin-left:20px">Cambiar</button>
					</div>
				</form>
				<form action="users.php" method="post"  class="clearfix">
					<div class="form-group clearfix">
						<button type="submit" name="back" class="btn btn-primary" style="margin-left:10px">Volver atras</button>
					</div>
				</form>
			</div>
		</div>
