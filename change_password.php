	...
	if(isset($_POST['update'])){
		...
		if(empty($errors)){
            if(sha1($_POST['old-password'])!==current_user()['password']){
				$session->msg('d',"La antigua contraseña no coincide.");
            ...
            if($result&&$db->affected_rows()===1):
				$session->logout();
                $session->msg('s',"Inicia sesión con la nueva contraseña.");
                redirect('index.php',false);
            else:
                $session->msg('d','Cambio de contraseña erróneo.');
                redirect('change_password.php',false);
            endif;
		}else{
...
<center><img src="libs/images/logo.png" alt="logo"></center>
