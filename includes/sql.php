<?php
	...
	function find_all($table, $condicion=""){
		global $db;
		if(empty($condicion)){
		    ...
		    if(tableExists($table)){
		        return find_by_sql("SELECT * FROM ".$db->escape($table)." WHERE ".$condicion);
		    }
		}
	}
	function find_viat(){
	    global $db;
	    return find_by_sql("SELECT * FROM val_viaticos WHERE dat_fecfin >= CURDATE();");     
	}
    ...
    function max_date($table, $field){
        global $db;
        if(tableExists($table)){
            return find_by_sql("SELECT MAX(".$db->escape($field).") AS fecha FROM ".$db->escape($table));
        }
    }
    function join_dept_table(){
        global $db;
        $sql="SELECT DISTINCT ciudad.str_depto FROM ciudad ORDER BY str_depto;";        
        return find_by_sql($sql);
    }
    function join_munid_table($id){
        $sql="SELECT DISTINCT str_nombre FROM ciudad WHERE str_depto = '$id' ORDER BY str_nombre";
        return find_by_sql($sql);
    }
	function join_product_table(){
		global $db;
		$sql="SELECT p.id,p.name,p.quantity,p.buy_price,p.sale_price,p.media_id,p.date,c.name";
		$sql.=" AS categorie,m.file_name AS image,p.id_supplier, s.nombre";
		$sql.=" FROM products p";
	...
	function join_project_table(){
	    global $db;
	    $sql="SELECT proyecto.id, str_nombre, id_cliente, nombre, dat_fecini, dat_fecfin, str_objeto";
	    $sql.=" FROM proyecto";
	    $sql.=" INNER JOIN clients ON proyecto.id_cliente = clients.id";
	    $sql.=" ORDER BY proyecto.dat_fecini DESC";
	    return find_by_sql($sql);
	}
	function join_valviat_table(){
	    global $db;
	    $sql="SELECT id, val_alimen, val_alimpar, val_hotel, val_lavand, dat_fecfin";
	    $sql.=" FROM val_viaticos";
	    $sql.=" ORDER BY dat_fecfin DESC";
	    return find_by_sql($sql);
	}
	function join_ot_table(){
	    global $db;
	    $sql="SELECT DAY( dat_fecgen) AS dia, MONTH(dat_fecgen) as mes, YEAR(dat_fecgen) as ano, ot.*, employees.nombre, employees.apellidos";
	    $sql.=" FROM ot";
	    $sql.=" INNER JOIN employees ON ot.id_empleado = employees.id";
	    $sql.=" ORDER BY ot.dat_fecgen DESC";
	    return find_by_sql($sql);
	}
	...
	function join_employees_table(){
		global $db;
		$sql="SELECT e.id,e.foto,e.nombre,e.apellidos,e.documento, e.puesto, e.ciudad, e.str_telcor, e.str_telper, e.str_emacor";
