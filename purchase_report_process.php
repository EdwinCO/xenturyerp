...
<?php
	if(isset($_POST['submit'])){
		...
		if(empty($errors)):
			...
			$results=find_purchase_by_dates($start_date,$end_date);
		else:
			$session->msg("d",$errors);
			redirect('purchases_report.php',false);
		endif;
	}else{
			...
			<table class="table table-border">
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-right"><?php echo remove_junk($result['total_purchases']); ?></td>
						<td class="text-right"><?php echo remove_junk($result['total_purchasing_price']); ?></td>
					</tr>
					<?php endforeach; ?>
				</tbody>
				<tfoot>
					...
					<?php
						$util=number_format(@total_price($results)[1],2)-$result['buy_price'];
					?>
					<tr class="text-right">
						<td colspan="5"></td>
						<td colspan="1">Utilidad</td>
						<td><?php echo number_format($util,2); ?> $</td>
					</tr>
				</tfoot>
			</table>
		</div>
